...
public class DateTimeParserTests
{
    ...
    [Theory]
    [InlineData("de-DE", "2025-12-24")]
    [InlineData("de-DE", "24.12.2025")]
    [InlineData("en-US", "12/24/2025")]
    [InlineData("ja-JP", "2025/12/24")]
    public void CanParse_WithValidDateTime_ShouldReturnTrue(string cultureName, string input)
    {
        var parser = new DateTimeParser();
        var culture = new CultureInfo(cultureName);
    ...
    [InlineData("en-US", "")]
    [InlineData("en-US", "32/12/2025")]
    [InlineData("en-US", "abc123")]
    public void CanParse_WithInvalidDateTime_ShouldReturnFalse(string cultureName, string input)
    {
        var parser = new DateTimeParser();
        var culture = new CultureInfo(cultureName);
        var result = parser.CanParse(input, culture);
